...
class Parser:
    ...
    def _expectet_token_error(self,token_type=TokenType)->None:
    ...
    def _expectet_token(self,token_type:TokenType)->bool:
        assert self._peek_token is not None
        if self._peek_token==token_type:
            self._advance_tokens()
            return True
        # Para agregar el error en el token
        self._expectet_token_error(token_type)
        return False
    ...
    def _parse_return_statement(self)->Optional[ReturnStatements]:
        ...
        #Cuando sepamos evaluar expresionaes para retornar
        self._advance_tokens()
        while self._current_token.token_type!=TokenType.SEMICOLON:
            self._advance_tokens()
